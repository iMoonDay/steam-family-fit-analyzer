...
- 这个工具适合自用参考，不建议把结果作为评价朋友、家人或伴侣“值不值得加入家庭”的依据。
- Steam 家庭共享涉及信任、名额、区域、冷却时间和彼此感受，游戏数量和价格不能代表一段关系的价值。
- 请尊重他人的隐私设置；目标账号游戏详情不公开时，脚本不会也不能绕过限制。
...
- 支持输入 SteamID64、Steam 好友码、`/profiles/<steamid64>`、`/id/<custom>` 或 vanity 名称；多个账号可用空格分隔，并按合并后的游戏库一起分析。
- 支持在 `steamcommunity.com/profiles/<steamid64>` 页面自动填入当前资料页账号。
- 统计总游戏、家庭库、统计进度、新增、重复、重复率和新增价值。
- 多账号分析时显示各账号拆分统计，可勾选要计入的账号，并在全部和新增列表标出目标拥有者。
- 分析时会同时读取当前登录账号的个人库，避免已在个人库但未进入家庭共享库的游戏被误算为新增。
- 列表包含全部、家庭库、新增、重复、搜索。
- 仅统计支持 Steam 家庭共享的游戏。
- 新增游戏原价按当前商店地区显示。
...
- 金额格式会根据商店地区显示。
- 免费或零价游戏不计入新增；无价格、下架或区域不可售游戏会显示为 `N/A`。
