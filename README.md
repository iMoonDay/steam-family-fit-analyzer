...
- Steam 商店接口可能触发 HTTP 429，脚本会暂停请求，稍后可手动继续。
- 价格模式默认为“快速”，会批量读取原价但不补中文名；“中文名”模式会逐个读取并补中文名，速度较慢。
...
- `ORIGINAL_PRICE_BATCH_SIZE`：快速价格模式下每批读取数量。
